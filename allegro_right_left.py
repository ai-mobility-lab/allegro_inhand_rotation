...
parser = argparse.ArgumentParser(description="Compare Allegro right and left hand URDFs")
...
asset_descriptors = [
    ...
    AssetDesc("assets/allegro/allegro_left.urdf"),
]
...
# assets are visually distinguishable.
# -----------------------------------------------------------------------------
def spawn_hand(prim_path: str, urdf_relpath: str, pos: tuple, color: tuple) -> Articulation:
    ...
    robot_cfg = ArticulationCfg(
        ...
        spawn=sim_utils.UrdfFileCfg(
            ...
            # DOF positions are teleported directly every frame -- collision would just
            # add spurious contact forces/jitter with no visual benefit here.
...
def get_dof_info(robot: Articulation, speed_scale: float, asset_name: str) -> dict:
    ...
    # Both Allegro URDFs are pure revolute joints with explicit <limit> tags --
    # unlike the original (a generic IsaacGym asset viewer), the
...
# -----------------------------------------------------------------------------
# Create one group with both hands, separated by 0.5m along y.
# -----------------------------------------------------------------------------
print("Creating 1 group with 2 actors")

right_hand = spawn_hand("/World/Right", asset_descriptors[0].file_name, (0.0, 0.25, 0.3), (0.3, 0.5, 1.0))
left_hand = spawn_hand("/World/Left", asset_descriptors[1].file_name, (0.0, -0.25, 0.3), (1.0, 0.5, 0.2))

sim.set_camera_view((1.5, 0.0, 0.5), (0.0, 0.0, 0.2))

sim.reset()

# -----------------------------------------------------------------------------
# Obtain DOF information for each asset.
# -----------------------------------------------------------------------------
dof_info0 = get_dof_info(right_hand, args_cli.speed_scale, asset_name=asset_descriptors[0].file_name)
dof_info1 = get_dof_info(left_hand, args_cli.speed_scale, asset_name=asset_descriptors[1].file_name)
...
all_robots = [right_hand, left_hand]
...
while simulation_app.is_running():
    ...
    right_hand.write_joint_state_to_sim(pos0, torch.zeros_like(pos0))
    left_hand.write_joint_state_to_sim(pos1, torch.zeros_like(pos1))

    if draw_interface is not None:
        draw_interface.clear_lines()
        p1_right, p2_right = dof_axis_line(right_hand, dof_info0["dof_names"][state0["current_dof"]], joint_axes0)
        p1_left, p2_left = dof_axis_line(left_hand, dof_info1["dof_names"][state1["current_dof"]], joint_axes1)
        draw_interface.draw_lines(
            [p1_right, p1_left], [p2_right, p2_left], [[1.0, 0.0, 0.0, 1.0]] * 2, [3.0, 3.0]
        )
